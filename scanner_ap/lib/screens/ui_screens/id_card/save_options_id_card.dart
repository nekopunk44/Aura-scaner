...
import '../passport/save_options_passport.dart' show SaveFormat;

class SaveOptionsIdCardScreen extends StatelessWidget {
  final List<String> sourceFilePaths;

  SaveOptionsIdCardScreen({super.key, required this.sourceFilePaths}) {
    assert(sourceFilePaths.length == 2, 'IdCardPhotoEditScreen должен передавать 2 файла.');
  }
  ...
  Future<String> _saveAsPdf(BuildContext context, String fileName) async {
    ...
    final image1File = File(sourceFilePaths[0]);
    final image2File = File(sourceFilePaths[1]);

    final image1Bytes = await image1File.readAsBytes();
    final image2Bytes = await image2File.readAsBytes();

    final pdfImage1 = pw.MemoryImage(image1Bytes);
    final pdfImage2 = pw.MemoryImage(image2Bytes);


    final cardWidth = PdfPageFormat.a4.availableWidth * 0.9;

    final cardHeight = cardWidth / 1.6;

    final requiredHeight = (cardHeight * 2) + 30;

    double finalCardHeight = cardHeight;
    double finalCardWidth = cardWidth;

    if (requiredHeight > PdfPageFormat.a4.availableHeight) {
      finalCardHeight = (PdfPageFormat.a4.availableHeight - 40) / 2.1;
      finalCardWidth = finalCardHeight * 1.6;
    }
        ...
        build: (pw.Context context) {
          return pw.Center(
            child: pw.Column(
              ...
              children: [
                // Изображение 1 (Лицевая сторона)
                pw.SizedBox(
                  width: finalCardWidth,
                  height: finalCardHeight,
                  // ИСПРАВЛЕНИЕ: Удален pw.Transform.rotate()
                  child: pw.Image(pdfImage1, fit: pw.BoxFit.contain),
                ),
                pw.SizedBox(height: 30),

                // Изображение 2 (Обратная сторона)
                pw.SizedBox(
                  width: finalCardWidth,
                  height: finalCardHeight,
                  child: pw.Image(pdfImage2, fit: pw.BoxFit.contain),
                ),
              ],
            ),
          );
  ...
  // --- Общая логика сохранения и навигации ---

  Future<void> _handleSave(BuildContext context, SaveFormat format) async {
    String defaultName = 'Scan_IDCard_${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}';
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Выберите формат сохранения (Удостоверение)',
              ...
              _buildOptionButton(
                context,
                icon: Icons.image,
                text: 'Сохранить как IMG (Лицевая сторона)',
